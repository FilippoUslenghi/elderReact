...
def populate_columns(yaw, pitch, roll, yaw_column, pitch_column, roll_column):

    yaw_column.append(yaw)
    pitch_column.append(pitch)
    roll_column.append(roll)
...
def draw_results_ssd(detected,input_img,faces,ad,img_size,img_w,img_h,model,time_detection,time_network,time_plot,yaw_column, pitch_column, roll_column):
    ...
    if detected.shape[2]>0:
        ...
        for i in range(0, detected.shape[2]):
            ...
            if confidence > 0.5:
                ...
                populate_columns(p_result[0][0], p_result[0][1], p_result[0][2], yaw_column, pitch_column, roll_column)
                break
                # face = face.squeeze()
        ...
        if not flag:
            populate_columns(np.nan, np.nan, np.nan, yaw_column, pitch_column, roll_column)
...
def main():
    ...
    for dataset in datasets:
        ...
        for csv in os.listdir(csv_dir):
            ...
            video_name = csv.replace('.csv', '.mp4')
            if csv in os.listdir(out_dir): continue
            ...
            while True:
                ...
                if img_idx==1 or img_idx%skip_frame == 0:
                    ...
                    draw_results_ssd(detected,input_img,faces,ad,img_size,img_w,img_h,model,time_detection,time_network,time_plot,yaw_column,pitch_column,roll_column)
            ...
            yaw_column = [(value+90)/180 for value in yaw_column]
            pitch_column = [(value+90)/180 for value in pitch_column]
            roll_column = [(value+90)/180 for value in roll_column]

            #populate the dataframe
            df['yaw'] = yaw_column
            df['pitch'] = pitch_column
            df['roll'] = roll_column
